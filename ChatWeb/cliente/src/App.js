...
function App() {
  ...
  useEffect(() => {
    ...
    const socket = new WebSocket("http://localhost:8080"); //ip red local cetys alumnos
    setWs(socket); //guardar la conexion en el estado

    //configurar para manejar eventos de la conexion
  ...
  return (
    <div>
      ...
      >
        {/* Mapeo de los mensajes recibidos para mostrarlos en el contenedor */}
        {messages.map((msg, index) => (
          <p key={index}>{msg}</p> // Cada mensaje se muestra en un párrafo, usando el índice como clave única
        ))}
      </div>
      <input
        type="text"
